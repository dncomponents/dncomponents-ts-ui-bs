...
import {ClickHandler, FilterHandler, KeyUpHandler, SelectionHandler} from 'dncomponents/lib/corecls/handlers';
...
export class SideMenuViewImpl implements SideMenuView {
    ...
    @UiStyle
    public showNavBarPanelStyle: string;
    @UiField
    ...
    private init() {
        ...
        this.searchBox.addHandler(KeyUpHandler.onKeyUp(evt => {
            this.tree.expandAll(this.searchBox.getValueFromView() != null);
            this.tree.drawData();
        }));
        ClickHandler.onClick(evt => {
            if (this.collapsedToggle) {
                this.navBtn.classList.remove(this.collapsedStyle);
                this.navbarPanel.classList.add(this.showNavBarPanelStyle);
            } else {
                this.navBtn.classList.add(this.collapsedStyle);
                this.navbarPanel.classList.remove(this.showNavBarPanelStyle);
            }
    ...
    public showFilter(b: boolean) {
        Util.setVisible(this.searchBox.asElement(), b);
    }
    ...
    public getValueFromView(): string {
        return this.searchBox.getValueFromView();
    }
