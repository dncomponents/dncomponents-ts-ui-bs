...
import {
    ...
    HeaderSorting,
    HeaderWithModifiers
} from 'dncomponents/lib/table/HeaderCellHolder';
...
import {DoubleBox} from 'dncomponents/lib/textbox/DoubleBox';
import {DateBox} from 'dncomponents/lib/textbox/DateBox';
...
export class FilterPanelViewImpl<T> implements FilterPanelView<T> {
    ...
    public setValueComponentStr<T1>(clzz: string) {
        if (clzz == null)
            throw new java.lang.NullPointerException('Class argument can\'t be null!');
        if (clzz === 'integer') {
            this.cellEditor = new DefaultCellEditor(new IntegerBox());
        } else if (clzz === 'double') {
            this.cellEditor = new DefaultCellEditor(new DoubleBox());
        } else if (clzz === 'string') {
            this.cellEditor = new DefaultCellEditor(new TextBox());
        } else if (clzz === 'date') {
            this.cellEditor = new DefaultCellEditor(new DateBox());
        } else if (clzz === 'boolean') {
            this.mainPanel.className = this.booleanMainPanelStyle;
            this.mainPanel.appendChild(this.ac.asElement());
            this.cellEditor = new DefaultCellEditor(new CheckBox());
        } else
            throw new java.lang.IllegalArgumentException(clzz + ' has no defined editor!');
    }
